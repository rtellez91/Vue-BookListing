...
describe("BookList.vue", () => {
  it("should contain book-form element with event call @book-list-contains-event-call-from-book-form", () => {
    ...
    assert(
      results.innerHTML.includes("bookform"),
      "BookList does not contain any `bookform` tags"
    );

    assert(
      results.innerHTML.includes('v-on:addbook="appendBook"') ||
        results.innerHTML.includes('@addbook="appendBook"'),
      "BookList does not contain a call to `appendBook` on the `addBook` event"
    );
  });
});
