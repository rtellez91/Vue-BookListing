...
describe("App.vue", () => {
  it("should include booklist in App.vue @app-vue-will-use-book-list", () => {
    ...
    results = document.querySelectorAll("div#app booklist");
    assert(
      results.length == 1,
      "There doesn't appear to be a single `booklist` element with opening and closing tags inside of a div in App.vue."
    );
    assert(
      results,
      "The `<booklist>` element does not exist inside of a div in App.vue."
    );
  });
});
